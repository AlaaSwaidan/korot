...
class AdminRepaymentExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithEvents
{
    /**
    ...
    */
    function __construct($data) {
        ...
        $this->type_repayment = $data->type_repayment;
        $this->type = $data->type;
    ...
    public function collection()
    {
        ...
        if ($this->type_repayment == "from_dis"){
            $data =$user->providerable()->where('type','repayment')->Order();
        }else{
            $data =$user->userable()->where('type','repayment')->Order();
        }
    ...
    {
        return [
            ['الى طرف','الرصيد','نوع التحويل','وصل التحويل','التاريخ']
        ];
    }
