...
class AdminCollectionExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithEvents
{
    /**
    ...
    */
    function __construct($data) {
        ...
        $this->type_collection = $data->type_collection;
        $this->type = $data->type;
    ...
    public function collection()
    {
        ...
        if ($this->type_collection == "from_dis"){
            $data =$user->providerable()->where('type','collection')->Order();
        }else{
    ...
    {
        return [
            ['من طرف','الرصيد','نوع التحويل','وصل التحصيل','التاريخ']
        ];
    }
    ...
    public function map($process): array
    {
        ...
        return [
            [
                getUserTypeModelName($process->userable_type) .' : '.$process->admin->name."(".$process->admin->email.")",
                $process->amount,
                $process->transfer_type == "fawry" ? "فوري" : "آجل",
                route('admin.collections.balance-merchant-pdf', $process->id),
