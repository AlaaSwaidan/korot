...
<table style="width:100%;border:none;">
    <tr>
        <td style="text-align:center;font-size: 20px">
            نوع التقرير (مبيعات الموزع {{ $distributor->name }})
            <br>
...
<table style="width:100%;border:1px solid #dee2e6;">
    ...
    @foreach($data as $value)
        @if($value->orders()->where('paid_order',"paid")->count() > 0)
            <tr style="text-align: center;border:1px solid #dee2e6;">
                <td style="text-align: center;border:1px solid #dee2e6;">#</td>
                <td style="text-align: center;border:1px solid #dee2e6;">{{ $value->name }}</td>
                <td style="text-align: center;border:1px solid #dee2e6;">{{ $value->orders()->where('parent_id','!=',null)->where('paid_order',"paid")->where('payment_method','online')->whereBetween(\DB::raw('DATE(created_at)'), [$startDate, $endDate])->sum('card_price') }}</td>
                <td style="text-align: center;border:1px solid #dee2e6;">{{ $value->orders()->where('parent_id','!=',null)->where('paid_order',"paid")->where('payment_method','wallet')->whereBetween(\DB::raw('DATE(created_at)'), [$startDate, $endDate])->sum('card_price') }}</td>
                <td style="text-align: center;border:1px solid #dee2e6;">{{ $value->orders()->where('parent_id','!=',null)->where('paid_order',"paid")->whereBetween(\DB::raw('DATE(created_at)'), [$startDate, $endDate])->sum('card_price') }}</td>
