...
@endsection
        @section('scripts')
            ...
            <script>
                $( "body" ).on( "change", "select[name='company_name']", function() {
                    var id = $(this).val();
                    if(id !== ''){
                        $('#packages').empty();
                        $.ajax({
                            url: '/admin/get/all-packages/'+id,
                            type: "GET",
                            dataType: "json",
                            success: function (data) {
                                ...
                                if (data['packages'].length > 0) {
                                    ...
                                    $('#packages').append('<select name="package_name" id="package_name" class="form-select form-select-solid" data-control="select2" data-placeholder="الباقات" data-hide-search="true">');


                                    $.each(data['packages'], function (index, packages) {


                                        $('select[name="package_name"]').append('<option  value="' + packages.id + '">' + packages.name_ar + '</option>');


                                    });
                                    $('#packages').append(' </select>');
            ...
            <script>
                ...
                var selected_url_delete = "{{ url('/') }}" + "/admin/merchants/profile-prices/selected/destroy" ;
